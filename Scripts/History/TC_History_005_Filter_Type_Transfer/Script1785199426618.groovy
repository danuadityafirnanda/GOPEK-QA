...
WebUI.comment('=== TC_History: Filter Type TRANSFER - START ===')
...
// ========================================
WebUI.comment('Step 4: Open Type Dropdown & Select Transfer')

// Klik Dropdown Type (Dropdown Ke-1)
clickTypeDropdown()
WebUI.delay(0.5)

// Pilih opsi 'Transfer' via Helper Radix UI
selectRadixOption('Transfer')
WebUI.comment('✅ Selected Type: Transfer')

// VALIDASI KUAT: Opsi terpilih harus tampil di trigger dropdown
TestObject selectedOptionTrigger = createDynamicObject("//button[@role='combobox' and contains(., 'Transfer')]")
boolean isSelectedVisible = WebUI.verifyElementPresent(selectedOptionTrigger, 5, FailureHandling.STOP_ON_FAILURE)
assert isSelectedVisible : "Validation failed: Selected option 'Transfer' not shown in dropdown trigger!"
...
WebUI.comment('✅ PASS: Filter Type Transfer applied successfully')

WebUI.takeFullPageScreenshot('Screenshots/History_Filter_Type_Transfer_Passed.png')
WebUI.comment('=== TC_History: Filter Type TRANSFER - COMPLETED ===')
